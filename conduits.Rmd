...
#We will not use length given in the data for now. SWMM can auto-calculate length from the xy coordinates of the nodes and that seems to work best for now, else you get error messages.

conduits <- conduits_data %>% 
  select( OBJECTID, ROUGHNESS, OBJECTID_12, RASTERVALU_1) %>% 
...
conduits[with(conduits, order(Name, Elevation)),]
...
match <- dcast(setDT(conduits), Name~rowid(Name, prefix="Nodes"), value.var="Nodes")


#the match function will only have the two variables. We re-merge data tables together using the merge function, Select the columns we want for SWMM, and remove any duplicates. 

distinct_conduits <- merge(match, conduits, by = "Name") %>% select(Name, Nodes2, Nodes1, Length, Roughness, InOffset, OutOffset, InitFlow, MaxFlow) %>% distinct() %>%

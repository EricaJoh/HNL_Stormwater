...
distinct_conduits_1 <- length %>% mutate(Roughness = if_else(is.na(Roughness), 1.18, Roughness)) %>%  filter(Nodes2 !="NA") %>% filter(Nodes1 !="NA") %>% select(Name, Nodes2, Nodes1, length_ft, Roughness, InOffset, OutOffset, InitFlow, MaxFlow, TYPE) %>% 
  distinct() 
  

# data frame for conduits file output:
distinct_conduits <- distinct_conduits_1 %>% select(Name, Nodes2, Nodes1, length_ft, Roughness, InOffset, OutOffset, InitFlow, MaxFlow)

...
```{r}
junctions_data <- structure_data %>% select(OBJECTID, INVELEV1, CATCHMENT_, DRAINAGE_A, POINT_X, POINT_Y) %>% 
  rename(Name = OBJECTID, Elevation = INVELEV1, MaxDepth = CATCHMENT_, Aponded = DRAINAGE_A, X_Coord = POINT_X, Y_Coord = POINT_Y)
...
```{r}
#arrange columns
junctions <- select(junctions_data, Name,Elevation ,MaxDepth, InitDepth, SurDepth,Aponded)
```

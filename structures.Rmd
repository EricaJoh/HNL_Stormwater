...
conduits <- conduit_data %>%  select( OBJECTID, OBJECTID_12, RASTERVALU, POINT_X, POINT_Y, CATCHMENT_, DRAINAGE_A, SLOPE) %>% 
  rename(Name = OBJECTID, Nodes = OBJECTID_12, Elevation = RASTERVALU, MaxDepth = CATCHMENT_, Aponded = DRAINAGE_A) 
...
Invert <- height %>% select(Nodes, Name, Elevation, height_ft)
write.csv(Invert,"Invert.csv", row.names = FALSE)
...
#arrange columns
junctions <- height %>% select( Nodes , Invert_elev,  MaxDepth, InitDepth, SurDepth, Aponded) %>% mutate(MaxDepth = if_else(is.na(MaxDepth), 0, MaxDepth)) %>% mutate(Aponded = if_else(is.na(Aponded), 0, Aponded))
...
coordinates <- structures %>% select (nod_Name, POINT_X, POINT_Y) %>% rename (Name = nod_Name ,X_Coord =  POINT_X , Y_Coord = POINT_Y) %>% distinct()
